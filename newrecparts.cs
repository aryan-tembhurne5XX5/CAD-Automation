...
namespace InventorPartExporterCombined
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Process Assembly 1 → Assembly 8
            for (int i = 1; i <= 8; i++)
            {
                string assemblyInput = System.IO.Path.Combine(baseInputRoot, $"Assembly {i}");
                string assemblyOutput = System.IO.Path.Combine(baseOutputRoot, $"assembly{i}");

                if (!System.IO.Directory.Exists(assemblyInput))
                {
                    Console.WriteLine($"Skipping missing folder: {assemblyInput}");
                    continue;
                }

                ProcessAllIPTs(assemblyInput, assemblyOutput);
            }

            Console.WriteLine("\nAll assemblies processed.");
        }

        static void ProcessAllIPTs(string inputRoot, string outputRoot)
        {
            foreach (string iptPath in System.IO.Directory.GetFiles(
                inputRoot,
                "*.ipt",
                System.IO.SearchOption.AllDirectories))
            {
                try
                {
                    ...
                    // ════════════════════════════════════════════════════════════════════
                    // 3. FACES & EDGES (NOW WITH B-REP ADJACENCY)
                    // ════════════════════════════════════════════════════════════════════
                ...
                {
                    Console.WriteLine($"Failed: {iptPath}");
        ...
        // ══════════════════════════════════════════════════════════════════════
        // EDGE EXTRACTION (NOW WITH B-REP ADJACENCY)
        // ══════════════════════════════════════════════════════════════════════
        static EdgeData ExtractEdge(Edge edge, ReferenceKeyManager mgr, int keyContext)
        {
            ...
            // ==========================================
            // TRUE B-REP ADJACENCY EXTRACTION
            // ==========================================
    ...
    public class EdgeData
    {
        ...
        // ==========================================
        // ADDED TO HOLD TOPOLOGY CONNECTIONS
        // ==========================================
